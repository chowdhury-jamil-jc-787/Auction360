...
<x-backend.layouts.master>
    <x-slot:fav>galleries-trashed</x-slot:fav>
        <x-slot:title>galleries-trashed</x-slot:title>
        @push('css')
        ...
        @endif
                  <div class="card mb-4">
                    ...
                    <a href="{{ route('galleries.index') }}" class="btn btn-success">Back</a>
                    </div>
            ...
            <tr>
                ...
                @endif
                  ...
                  <td>{{ $gallery->created_at->longAbsoluteDiffForHumans() }}</td>
                <td class="text-nowrap">
                    <form action="{{ route('galleries.trashed.delete',$gallery->id) }}" method="POST">
                        @can('gallery-trashed-restore')
                        <a class="btn btn-primary" href="{{ route('galleries.trashed.restore',$gallery->id) }}">restore</a>
                        @endcan
                        @csrf
                        @method('DELETE')
                        @can('gallery-trashed-delete')
